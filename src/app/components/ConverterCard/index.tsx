...
export const ConvertCard = ({defaultValue, index, id}: {defaultValue: string, index : number, id: string}) => {
  ...
  const firstPrices = data.length > 1 &&  data.find(obj => obj.id === id)?.data.prices;
  const currentPrice = coins && firstPrices && firstPrices.length > 0 && firstPrices[firstPrices.length-1][1];
  ...
  return (
    <div className={`w-10/12 md:w-1/2 h-36  md:h-52  bg-white m-5 p-3 md:p-6 rounded-2xl flex flex-col justify-between ${index ? `dark:bg-light-text-color-two`:`dark:bg-purplea`}`}>
    ...
    <div>
      ...
      <div className='font-normal text-sm text-purpleb p-2 dark:text-white '>1 {cryptoSymbol} = {currentPrice && symbol + currentPrice.toFixed(2)}</div>
      <div></div>
    </div>
  </div>
  )
