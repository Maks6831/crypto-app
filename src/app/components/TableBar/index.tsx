...
export const TableBar = ({number, numerator, denominator, change1h, isFirst}:{number:number, numerator: number, denominator: number, change1h: number, isFirst: boolean}) => {
  const reduxSymbol = useAppSelector(state => state.currencyReducer.symbol);
  const { theme } = useTheme()

  return (
    <td className={`p-5 hidden ${isFirst ? ' sm:table-cell ' : 'lg:table-cell '} bg-white dark:bg-dark-card h-full `}>
